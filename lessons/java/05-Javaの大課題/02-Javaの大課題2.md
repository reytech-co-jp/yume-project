...
その入力例の値のように入力して期待された出力例が出力するようにしましょう。   
...
そこで、期末テストの点数の各位の数を足した数の一の位(文字列の二つ目の値)で勝負することにし、大きい方が勝ちとしました。   
つまり、<strong>`85`点であれば`8 + 5 = 13`で、`13`の一の位(二つ目の値)の`3`となります。</strong>   
...
75 81
```
Leonの点数は`75`から`7`と`5`の足し算は`12`ですが問題の通りだと一位の値(二つ目の値)`2`を取ります。それからAnnaの点数は`81`から`8`と`1`の足し算は`9`です。
...
String型からint型に変換することについて参考リンク: https://www.javatpoint.com/java-integer-parseint-method
...
- <strong>足した数は`10`未満</strong>であればその足した値をそのまま取ります。
  - <strong> `34`点であれば`3 + 4 = 7`で、`7`となります。</strong>   
- <strong>足した数は`10`以上</strong>の場合は問題で説明された通り一の位(二つ目の値)をとります。
  - <strong>`85`点であれば`8 + 5 = 13`で、`13`の一の位(二つ目の値)の`3`となります。</strong>    

<strong>※LeonとAnnaの点数`10`点より小さい点は`0`となります。</strong>   
<strong>※LeonとAnnaの点数`100`点は最初の位の`1`となります。</strong>   

入力例を利用して自分のコードが出力例と同じく出力を出してください。入力例は3つありますので、その3つを試してみてください。   
...
※完了したら自分のレポジトリにプッシュしプールリクエストを作成してSlackにレビューを出してください。
