...
文字列の空白の数：3空白
```
ここで使う`String`クラスの「length()」と「charAt()」メソッドを知らない方はこちらのサイトを参考にしてください。  
...
fun
```
ここでは`String`クラスの「split()」メソッドを使用しますのでこのメソッドについて知らない方はこちらのサイトを参考にしてください。  
...
```
この課題では単語を大文字にする`String`クラスの「toUpperCase()」メソッドと文字列を切り出し抽出する`String`クラスの「substring()」メソッドを使用しますのでこれらについて知らない方はこちらのサイトを参考にしてください。  
